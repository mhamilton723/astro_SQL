If my.sdss_dr8objid IN(select original_sdss_dr8objid from mydb.$$SDSS_OUT$$_2)
select distinct 

my.survey, my.xray_objid,
  my.sdss_dr8objid as original_sdss_dr8objid, 
  s.objid as new_sdss_dr8objid,
d.thingid as thingid,
from $$SDSS_OUT$$_2 my
where my.date_diff = (select min(date_diff) from $$SDSS_OUT$$_2 as f where f.original_sdss_dr8objid = my.original_sdss_dr8objid)
...
my.survey, my.xray_objid,
my.sdss_dr8objid as original_sdss_dr8objid, 
my.sdss_dr8objid as new_sdss_dr8objid, 
my.thingid,
my.RA, my.DEC, my.z, my.class,my.SOFT_FLUX,my.hard_flux,my.full_flux,my.SDSS_REJ,my.WISE_REJ,
...
into mydb.$$SDSS_OUT$$  from Dr10.PhotoObj s
JOIN mydb.$$SDSS_OUT$$_1 my ON s.objid = my.sdss_objid
