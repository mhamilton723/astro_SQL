...
$baseCurlCmd='curl http://surveys.roe.ac.uk:8080/wsa/WSASQL  -F "formaction=freeform" -F "database=UKIDSSDR9PLUS" '.
    ' -F "sqlstmt=<queries/out_MW/sql2.txt" -F "iFmt=CSV" -F "format=CSV" -F "compress=GZIP" -F "rows=1" -F "timeout=10800"';

$fileExtension=".csv.gz"; # extension used to search output, change if you change outpur format format=CSV, compress=GZIP

$uploadFile="data/S82T_sdssfix.csv"; # file to load as #usertbale
...
$resultsFile="S82T_ukidss_raw".$fileExtension; # file to down load results to
print "$curlCmd \n";
